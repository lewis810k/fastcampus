...
접근이 원활하지 않으면 preferred DNS를 바꾸면 원활히 되는 경우도 있다. 
...
10진수에서 1은 1볼트, 2는 2볼트, 3은 3볼트.. 이런 형식으로 해서 신호를 숫자로 변환할 수도 있지만, 모든 도체는 저항을 가지고 있기 때문에 신호가 약해질 수 있다. 3볼트로 보내지만 실제로 도달할 때는 3볼트로 도착한다고 보장할 수 없다. 하지만 0과 1을 사용할 경우, 도달하기 전에 증폭기를 통해 신호를 증폭시킨다. 조금이라도 있으면 1로 보고 아예 없으면 0으로 인식한다. 
